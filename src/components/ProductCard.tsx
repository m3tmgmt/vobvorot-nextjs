...
const ProductCard = memo(function ProductCard({ 
  ...
  const productData = useMemo(() => {
    ...
    // Apply real-time stock adjustments based on reservations
    const skusWithAdjustedStock = product.skus.map(sku => {
      const adjustedStock = getAdjustedStock(sku.id, sku.stock, sku.reservedStock)
      return {
        ...sku,
        reservedStock: adjustedStock.reservedStock,
        availableStock: adjustedStock.availableStock
      }
    })
    
    const inStock = skusWithAdjustedStock.some(sku => sku.availableStock > 0)
    const totalAvailableStock = skusWithAdjustedStock.reduce((total, sku) => total + sku.availableStock, 0)
    const defaultSku = skusWithAdjustedStock[0]
  ...
  }, [product.images, product.skus, getAdjustedStock])

  // Memoize click handlers
  const handleAddToCart = useCallback(async () => {
    const { defaultSku, primaryImage } = productData
    if (defaultSku && defaultSku.availableStock > 0) {
      setIsLoading(true)
      try {
        dispatch({
          type: 'ADD_ITEM',
          payload: {
            ...
            maxStock: defaultSku.availableStock
          }
        })
  ...
  const { primaryImage, minPrice, inStock, totalAvailableStock, defaultSku } = productData
      ...
      <div className="product-info">
        ...
        <div style={{
          ...
          <p className="product-stock" style={{ 
            ...
            {inStock ? (totalAvailableStock === 1 ? 'last one!' : `${totalAvailableStock} left`) : 'sold out'}
          </p>
        </div>
