...
export const orderItemSchema = z.object({
  productId: z.string().refine(
    (val) => val.startsWith('mock-product-') || z.string().uuid().safeParse(val).success,
    'Некорректный ID продукта'
  ),
  skuId: z.string().refine(
    (val) => val.startsWith('mock-sku-') || z.string().uuid().safeParse(val).success,
    'Некорректный ID SKU'
  ),
